...
# 当前 pitch 相对正常姿态增加超过该角度时，认为正在低头。
LOW_HEAD_DELTA_DEG = 15.0
# ---------- 真人/照片判定的可调参数 ----------
...
def head_pitch_degrees(
    pixels: list[np.ndarray], camera_matrix: np.ndarray
) -> Optional[float]:
    """使用人脸关键点和 solvePnP 估计头部 pitch 角。

    solvePnP 需要两组对应点：
    1. model_points：人脸在一个简化三维模型中的坐标；
    2. image_points：这些三维点投影到当前图像后的二维像素坐标。

    相机内参 camera_matrix 描述焦距和主点位置。求解后通过旋转矩阵分解
    得到欧拉角，函数返回 pitch。这里的角度方向与 OpenCV 的姿态坐标约定
    有关，所以程序通过按 N 键记录正常姿态，再使用相对变化判断低头。
    """
    # 选取鼻尖、下巴、左右眼角、左右嘴角等稳定的人脸关键点。
    image_points = np.array(
        [pixels[1], pixels[152], pixels[33], pixels[263], pixels[61], pixels[291]],
        dtype=np.float64,
    )
    # 一个近似的人脸三维模型，单位为毫米。它不要求与真实人脸完全一致，
    # 只要各关键点之间的相对空间关系合理即可用于姿态估计。
...
def main() -> None:
    ...
    blink = BlinkDetector()
    neutral_pitch: Optional[float] = None
        ...
        with mp_face_mesh.FaceMesh(
            ...
            while True:
                ...
                if result.multi_face_landmarks:
                    ...
                    # 根据六个人脸关键点估计头部 pitch，并与校准姿态比较。
                    pitch_value = head_pitch_degrees(pixels, camera_matrix)
                    if pitch_value is not None:
                        if neutral_pitch is None:
                            head_status = "待校准（按 N 键）"
                        elif pitch_value - neutral_pitch > LOW_HEAD_DELTA_DEG:
                            head_status = "低头"
                        else:
                            head_status = "正常"
                ...
                pitch_text = "未校准" if pitch_value is None else f"{pitch_value:.1f} 度"
                ...
                cv2.imshow("课题A - D455 彩色与深度", display)

                # waitKey 同时负责刷新窗口和读取键盘输入。
                key = cv2.waitKey(1) & 0xFF
                if key == ord("n") and pitch_value is not None:
                    # 用户保持正常正视姿态按 N，把当前 pitch 作为个人基准。
                    neutral_pitch = pitch_value
                    print(f"已记录正常头部俯仰角：{neutral_pitch:.2f} 度")
                elif key == ord("s"):
